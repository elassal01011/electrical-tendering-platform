...
import {
  ...
  parsePricingHeaders,
  valueAt,
} from "../../src/lib/services/pricing/pricingImport";
...
function sheetWith(
  ...
  headers.forEach((value, index) => {
    if (value !== undefined) header.getCell(index + 1).value = value;
  });
  rows.forEach((values, rowIndex) =>
    values.forEach((value, columnIndex) => {
      if (value !== undefined)
        sheet.getRow(rowIndex + 2).getCell(columnIndex + 1).value = value;
    }),
  );
  return sheet;
}
...
describe("pricing XLSX header parsing", () => {
  it("maps normal template headers to 1-based ExcelJS columns", () => {
    const parsed = parsePricingHeaders(
      sheetWith([
        "Supplier",
    ...
    expect([...parsed.columnMap.values()].every((column) => column >= 1)).toBe(
      true,
    );
  ...
  it("normalizes spaces, underscores, hyphens, and capitalization", () => {
    const parsed = parsePricingHeaders(
      sheetWith([" supplier ", "MANUFACTURER", "part_number", "unit-price"]),
    );
    expect(parsed.missing).toEqual([]);
  });
  ...
  it("reports missing required headers clearly", () => {
    ...
    expect(parsed.missing).toEqual([
      "Part Number, Supplier SKU, or Description",
    ]);
  });
  ...
  it("ignores blank columns, blank rows, and unrelated columns", () => {
    const sheet = sheetWith(
      ...
      [
        ...
        ["Acme", undefined, "ABB", "A1", 10, "ignored"],
      ],
    );
  ...
  it("never calls row.getCell(0) when an optional column is absent", () => {
    const sheet = sheetWith(
      ["Supplier", "Manufacturer", "Part Number", "Unit Price"],
      [["Acme", "ABB", "A1", 10]],
    );
    const parsed = parsePricingHeaders(sheet);
    ...
    const calls: number[] = [];
    row.getCell = ((column: number) => {
      calls.push(column);
      return original(column);
    }) as typeof row.getCell;
  ...
  it("normalizes supplier names without merging meaningful characters", () => {
    expect(normalizePricingIdentity("  ACME--Electrical,   Ltd. ")).toBe(
      "acme electrical ltd",
    );
    expect(normalizePricingIdentity("ACME Electrical Ltd")).toBe(
      "acme electrical ltd",
