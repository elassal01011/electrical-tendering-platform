...
export function detectHeaderRow(rows: unknown[][]): number {
  ...
  rows.slice(0, 30).forEach((row, index) => {
    const detected = detectColumns(row).filter(
      (column) => column.confidence === 100,
    );
    const score = detected.reduce(
      (total, column) =>
        total + (["description", "quantity"].includes(column.field) ? 5 : 1),
      0,
    );
    if (score > bestScore) {
      best = index;
...
export function detectColumns(header: unknown[]): Detection[] {
  ...
  return header.map((raw, index) => {
    ...
    let field: BoqField = "ignore";
    let confidence = 0;
    for (const [candidate, aliases] of Object.entries(ALIASES) as [
      Exclude<BoqField, "ignore">,
      string[],
    ][]) {
      const exact = aliases.includes(value);
      const partial =
        !exact && aliases.some((a) => value.includes(a) || a.includes(value));
      const candidateConfidence = exact
        ? 100
        : partial && value.length > 2
          ? 72
          : 0;
      if (candidateConfidence > confidence && !used.has(candidate)) {
        field = candidate;
