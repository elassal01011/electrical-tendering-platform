...
export function netComponentCost(listPrice: number, supplierDiscountPct: number): number {
  return listPrice * (1 - supplierDiscountPct / 100);
}

export function materialCost(items: LineItemInput[]): number {
  return items.reduce((sum, item) => sum + netComponentCost(item.listPrice, item.supplierDiscountPct) * item.quantity, 0);
}
...
export function overheadCost(material: number, labor: number, overheadPct: number): number {
  return (material + labor) * (overheadPct / 100);
}

export function costBeforeMargin(material: number, labor: number, overhead: number): number {
  return material + labor + overhead;
}

/** Selling price computed via markup on cost. */
export function sellingPriceFromMarkup(cost: number, markupPct: number): number {
  return cost * (1 + markupPct / 100);
}

/** Selling price computed via target gross margin (margin as % of selling price, not cost). */
export function sellingPriceFromGrossMargin(cost: number, grossMarginPct: number): number {
...
export function grossMarginPctFromPrices(sellingPrice: number, cost: number): number {
  if (sellingPrice === 0) return 0;
  return (profit(sellingPrice, cost) / sellingPrice) * 100;
}

export function markupPctFromPrices(sellingPrice: number, cost: number): number {
  if (cost === 0) return 0;
...
export function computePricingSummary(input: PricingSummaryInput): PricingSummary {
  ...
  if (input.mode === "MARKUP") {
    ...
    if (input.grossMarginPct == null) throw new Error("grossMarginPct is required when mode is GROSS_MARGIN");
