...
export async function GET() {
  ...
  sheet.addRow(["Example Supplier", "SUP-001", "Example Manufacturer", "PART-001", "Optional description", "EA", "EGP", 125.5, new Date(), "", "true", "Optional source"]);
  sheet.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };
  sheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1E3A5F" } };
  sheet.getRow(1).alignment = { vertical: "middle" };
  sheet.views = [{ state: "frozen", ySplit: 1 }];
  sheet.columns = PRICING_TEMPLATE_HEADERS.map((header, index) => ({ header, width: index === 4 || index === 11 ? 24 : 20 }));
